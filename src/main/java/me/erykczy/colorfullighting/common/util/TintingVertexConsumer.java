...
public class TintingVertexConsumer implements VertexConsumer {
    ...
    @Override public VertexConsumer color(int r, int g, int b, int a){
        int rr = clamp(Math.round(r * mr));
        int gg = clamp(Math.round(g * mg));
        int bb = clamp(Math.round(b * mb));
        return d.color(rr, gg, bb, a);
    }
    ...
    @Override public void defaultColor(int r, int g, int b, int a){ d.defaultColor(r,g,b,a); }
